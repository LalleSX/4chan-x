...
(async () => {
  try {
    const packageJson = await readJsonFile(resolve(__dirname, '../package.json'));
    const fileName = generateFileName(packageJson.meta.path, channel, minify, '.user.js');
    const metaFileName = generateFileName(packageJson.meta.path, channel, minify, '.meta.js');
      ...
      plugins: [
        ...
        inlineFile({
          ...
          transformer(css) {
            ...
            return css.replace(/^ {2,}([a-z\-]+:) +/gm, '$1').replace(/\r?\n[ \t+]*/g, '').replace(/;\}/g, '}').replace(/ \{/g, '{').replace(/\/\*[^\*]*\*\//g, '').replace(/ ([>+~]) /g, '$1');
          }
        }),
