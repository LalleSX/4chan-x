...
import { g, Conf, c } from '../globals/globals.js'
...
const Test = {
  init() {
    ...
    if (Conf['Menu']) {
      const a = $.el('a', { textContent: 'Test HTML building' })
    ...
    const a2 = $.el('a', { textContent: 'Test HTML building' })
    ...
    if (Unread.posts) {
      const testOrderLink = $.el('a', { textContent: 'Test Post Order' })
  ...
  normalize(root) {
    ...
    for (el of $$('a[href]', root2)) {
      let { href } = el
      href = href.replace(
        /(^\w+:\/\/boards\.4chan(?:nel)?\.org\/[^/]+\/thread\/\d+)\/.*/,
        '$1'
      )
      ...
      function () {
        ...
        for (const postData of posts) {
          if (postData.no === post.ID) {
            ...
            Test.time += t2 - t1
            const post2 = new Post(root, post.thread, post.board, {
              forBuildTest: true,
            })
            let fail = false
  ...
  testAll() {
    g.posts.forEach(function (post) {
      if (!post.isClone && !post.isFetchedQuote) {
        ...
        if (
          !(
            (abbr = $('.abbr', post.nodes.comment)) &&
            /Comment too long\./.test(abbr.textContent)
          )
        ) {
  ...
  report() {
    if (Test.postsFailed) {
      new Notice(
        'warning',
        `${Test.postsFailed} post(s) differ (${Test.time} ms)`,
        30
      )
    } else {
  ...
  cb: {
    ...
    testOrder() {
      ...
      const list2 = (() => {
        const result1 = []
        for (x of $$(
          (g.SITE.isOPContainerThread ? `${g.SITE.selectors.thread}, ` : '') +
            g.SITE.selectors.postContainer
        )) {
