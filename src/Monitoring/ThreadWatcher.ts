...
import UnreadIndex from './UnreadIndex.js'
...
const ThreadWatcher = {
  ...
  init() {
    ...
    $.on(window, 'visibilitychange focus', () =>
      $.queueTask(ThreadWatcher.fetchAuto)
    )
  ...
  fetch(url, { siteID, force }, args, cb) {
    ...
    const req = $.whenModified(url, 'ThreadWatcher', onloadend, {
      timeout: MINUTE,
      ajax,
    })
  ...
  fetchAuto() {
    ...
    const now = Date.now()
    if (
      (now - interval >= (middle = db.data.lastChecked || 0) || middle > now) &&
      !d.hidden &&
      !!d.hasFocus()
    ) {
  ...
  fetchAllStatus(interval = 0) {
    ...
    const dbs = [ThreadWatcher.db, ThreadWatcher.unreaddb, QuoteYou.db].filter(
      x => x
    )
    let n = 0
    return dbs.map(dbi =>
  ...
  fetchBoard(board, deep) {
    ...
    const urlF =
      deep && site.threadModTimeIgnoresSage ? 'catalogJSON' : 'threadsListJSON'
    ...
    if (this.status === 200 && this.response) {
      ...
      for (const postObj of this.response.posts) {
        ...
        if (!unread || (!quotingYou && quotesYou)) {
          if (
            Filter.isHidden(site.Build.parseJSON(postObj, { siteID, boardID }))
          ) {
            continue
          }
        }
    ...
    addMenuEntries() {
      ...
      entries.push({
        ...
        open() {
          this.el.classList.toggle(
            'disabled',
            !$('.replies-unread', ThreadWatcher.list)
          )
          return true
        },
      })
      ...
      entries.push({
        ...
        open() {
          this.el.classList.toggle(
            'disabled',
            !$('.dead-thread', ThreadWatcher.list)
          )
          return true
        },
      })
      ...
      entries.push({
        ...
        open() {
          this.el.classList.toggle(
            'disabled',
            !$.hasClass(ThreadWatcher.shortcut, 'replies-quoting-you')
          )
          return true
        },
      })
